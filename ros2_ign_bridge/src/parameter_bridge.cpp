...
int main(int argc, char * argv[])
{
  ...
  // ROS 2 node
  auto ros2_node = std::make_shared<rclcpp::Node>("ros2_ign_bridge");

  // Ignition node
  auto ign_node = std::make_shared<ignition::transport::Node>();

  std::list<ros2_ign_bridge::BridgeHandles> all_handles;
  ...
  for (auto i = 1; i < argc; ++i) {
    ...
    std::string ros2_type_name = arg.substr(0, delimPos);
    arg.erase(0, delimPos + delim.size());
    ...
    try {
      ros2_ign_bridge::BridgeHandles handles =
        ros2_ign_bridge::create_bidirectional_bridge(
        ros2_node, ign_node,
        ros2_type_name, ign_type_name,
        topic_name, queue_size);
      all_handles.push_back(handles);
    } catch (std::runtime_error & _e) {
      std::cerr << "Failed to create a bridge for topic [" << topic_name << "] " <<
        "with ROS2 type [" << ros2_type_name << "] and " <<
